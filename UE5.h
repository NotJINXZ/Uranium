...
static UObject* FindObject(std::string name, bool bUseFullName = true)
{
...
//Credits: https://github.com/NeoniteDev/NeonitePP/blob/main/Cranium/finder.h
namespace Offsets
{
...
static GameObject* InternalFindChildInObject(GameObject* inObject, std::string childName)
{
	...
	while (next)
	{
		std::string nextName = reinterpret_cast<FField*>(next)->GetName();
...
{
	auto Class = (UClass*)FindObject(ClassName, true);
	//std::cout << "ClassName: " << Class->GetFullName() << std::endl;
